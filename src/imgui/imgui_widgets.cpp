...
static float linear_to_srgb(const float &linear) {
    float res = linear <= 0.0031308f ? linear * 12.92f : 1.055f * powf(linear, 1.f / 2.4f) - 0.055f;
    return res;
}

static float srgb_to_linear(const float &srgb) {
    float res = srgb <= 0.04045f ? srgb / 12.92f : powf((srgb + 0.055f) / 1.055f, 2.4f);
    return res;
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    f[0] = linear_to_srgb(f[0]);
    f[1] = linear_to_srgb(f[1]);
    f[2] = linear_to_srgb(f[2]);
    ...
    bool value_changed_as_hsv = false;
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            else
            {
                ...
                value_changed_as_hsv |= fmt_idx == 2;
            }
            if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    if (value_changed && picker_active_window == NULL)
    {
        if (!value_changed_as_float) {
            for (int n = 0; n < 4; n++) {
                f[n] = i[n] / 255.0f;
                f[n] = (value_changed_as_hsv || n == 3) ? f[n] : srgb_to_linear(f[n]);
            }
        }
